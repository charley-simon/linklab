...
export class CapacityBaselineStore {
  ...
  private baseline: CapacityBaseline | null = null

  /** Fenêtre glissante des mesures de throughput (rps observés) */
  private throughputSamples: number[] = []
  private readonly windowSize = 60   // 60 secondes de mesures
   ...
   */
  nominalRps(): number {
    if (this.baseline) return this.baseline.nominalRps

    if (this.throughputSamples.length === 0) return 100  // fallback initial
