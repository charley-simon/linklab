...
export class DuckDBDriver implements TelemetryDriver {
  private readonly dbPath: string
  private readonly maxRows: number

  private duckdb: any = null // module @duckdb/node-api
  private instance: any = null // DuckDBInstance
  private conn: any = null // DuckDBConnection
  ...
  async readRecent(limit: number): Promise<Span[]> {
    return this.query(`SELECT * FROM spans ORDER BY timestamp DESC LIMIT ${limit}`)
  }

  async readErrors(limit: number): Promise<Span[]> {
    return this.query(
      `SELECT * FROM spans WHERE has_error = true ORDER BY timestamp DESC LIMIT ${limit}`
    )
  }

  async readByTrail(trail: string, limit: number): Promise<Span[]> {
    return this.query(
      `SELECT * FROM spans WHERE trail = ${this.str(trail)} ORDER BY timestamp DESC LIMIT ${limit}`
    )
  }

  async aggregate(windowMs: number): Promise<SystemMetrics> {
    if (!this._connected || !this.conn) return this.emptyMetrics(windowMs)
    ...
    if (!rows.length) return this.emptyMetrics(windowMs)

    const [total, errors, hits, misses, yoyo, , p90] = rows[0]

    const totalSpans = Number(total) || 0

    if (totalSpans === 0) return this.emptyMetrics(windowMs)
  ...
  async latencyPercentiles(
    windowMs: number
  ): Promise<Array<{ route: string; p50: number; p90: number; p99: number; count: number }>> {
    if (!this._connected) return []
  ...
  async yoyoRateByRoute(windowMs: number): Promise<Array<{ route: string; yoyoRate: number }>> {
    if (!this._connected) return []
  ...
  async unstableTrails(
    windowMs: number,
    minVariants = 2
  ): Promise<Array<{ trail: string; pathVariants: number }>> {
    if (!this._connected) return []
  ...
  private async initSchema(): Promise<void> {
    ...
    await this.conn.run(
      `CREATE INDEX IF NOT EXISTS idx_spans_route     ON spans (from_node, to_node)`
    )
   ...
   */
  private async queryRaw(sql: string): Promise<any[][]> {
    if (!this._connected || !this.conn) return []
    try {
  ...
  private async query(sql: string): Promise<Span[]> {
   ...
   */
  private readerToRows(reader: any): any[][] {
    ...
    if (typeof reader.getRowObjects === 'function') {
      const objs: Record<string, any>[] = reader.getRowObjects()
  ...
  private rowToSpan(row: any[]): Span {
    const [
      ...
      filtersJson,
      totalMs,
      rowCount, // cache_hits, cache_misses, yoyo_events
      ,
      ,
      ,
      hasError,
    ...
    return {
      ...
      error:
        hasError && errorType
          ? { type: String(errorType), message: String(errorType), engineState: {} as any }
          : undefined,
  ...
  private async maybeRotate(): Promise<void> {
    ...
    const count = Number(rows[0]?.[0]) || 0
    if (count > this.maxRows) await this.rotate()
  }
  ...
  private emptyMetrics(windowMs: number): SystemMetrics {
    return {
      window: windowMs,
      timestamp: Date.now(),
      tension: 1,
