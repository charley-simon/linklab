...
export interface GraphDriverOptions {
  maxSpans?: number   // défaut : 10_000
}

export class GraphDriver implements TelemetryDriver {
  ...
  byRoute(from: string, to: string, limit = 100): Span[] {
    return this.spans
   ...
   */
  latencySamples(route: string, limit = 100): number[] {
...
function aggregateSpans(spans: Span[], windowMs: number): SystemMetrics {
  ...
  for (const s of spans) {
    for (const e of s.cacheEvents) {
      if (e.hit) cacheHits++; else cacheMisses++
      if (e.yoyo) yoyoEvents++
    }
  }
  ...
  for (const s of spans) {
    if (!byTrail.has(s.trail)) byTrail.set(s.trail, new Set())
    byTrail.get(s.trail)!.add(s.path.join('→'))
  }
  let stable = 0
  for (const paths of byTrail.values()) if (paths.size === 1) stable++
