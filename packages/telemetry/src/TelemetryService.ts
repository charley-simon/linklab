...
import type { Span, SystemMetrics } from './types.js'
...
export class TelemetryService {
  ...
  private readonly calibration?: CalibrationJob
  ...
  private readonly duckdbEnabled: boolean
  private started = false
  private readonly _onSpanEnd:   (span: Span) => void
  private readonly _onSpanError: (span: Span) => void
  ...
  constructor(opts: TelemetryServiceOptions = {}) {
    ...
    this._onSpanEnd   = (span: Span) => this.onSpan(span)
    this._onSpanError = (span: Span) => this.onSpan(span)
  }
  ...
  async start(): Promise<void> {
    ...
    console.log(
      `[Telemetry] Démarré — fenêtre ${this.windowMs}ms` +
  ...
  private onSpan(span: Span): void {
    ...
    if (this.duckdb.isConnected) this.duckdb.write(span)
...
function formatMetric(value: number, type: 'tension' | 'ratio'): string {
