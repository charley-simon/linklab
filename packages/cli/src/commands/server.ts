...
export async function server(options: ServerOptions = {}): Promise<void> {
  ...
  try {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (fs.existsSync(candidate)) { dotenvMod.config({ path: candidate }); break }
...
console.log('movie count:', dataset['movie']?.length ?? 0)
  ...
  try {
    ...
    console.error(c(C.red, `\n  ✖  Impossible de démarrer le serveur : ${(e as Error).message}\n`))
    process.exit(1)
  }

  // ── Output ────────────────────────────────────────────────

  const entities: string[] = compiled.nodes.map((n: any) => n.id)
  const routeCount: number = compiled.routes?.length ?? 0
