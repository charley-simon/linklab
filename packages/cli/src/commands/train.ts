...
export async function train(options: { alias?: string } = {}): Promise<void> {
  ...
  const testReport: TestReport = JSON.parse(fs.readFileSync(testGenPath, 'utf-8'))
  ...
  for (const result of testReport.results) {
    ...
    if (!result.path || !Array.isArray(result.path) || result.path.length === 0) continue
    ...
    const ucResult = result as any
    const pathKey =
      ucResult.type === 'composed' && ucResult.semantic
  ...
  const metricsMap = new Map(Object.entries(metrics).map(([k, v]) => [k, v])) as any
  ...
  try {
    ...
    ;(newCompiled as any).version = bumpPatch((compiled as any).version ?? '2.0.0')
    ;(newCompiled as any).alias = alias
  ...
  log.step(
    5,
    'Compile',
    `${compileStats.totalRoutes} routes (${compileStats.physical} physical · ${compileStats.semantic} semantic · ${compileStats.composed ?? 0} composed)`,
    d
  )

  const prevVersion = (compiled as any).version ?? '2.0.0'
  const newVersion = bumpPatch(prevVersion)

  log.warnings([])
  log.success(
    path.relative(cwd, compiledPath).replace(/\\/g, '/'),
