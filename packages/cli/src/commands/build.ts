...
import type { BuildOptions, Warning, StepResult } from '../types.js'
...
import type { MetricsMap, TrainingMetrics, UseCase, GraphData } from '@linklab/core'
...
function readVersion(compiledPath: string): string {
  try {
...
function defaultUseCases(graph: GraphData): UseCase[] {
...
export async function build(options: BuildOptions = {}): Promise<void> {
  ...
  const { durationMs: d1 } = await timedAsync(async () => {
    ...
    try {
      if (config.source.type === 'json') {
        ...
        const provider = new PostgresProvider(config.source as any)
  ...
  const advices = (analyzedSchema?.advices ?? []) as any[]
  const pivots = advices.filter((a: any) => a.action === 'SUGGEST_VIRTUAL_VIEWS').length
  const warnAdvices = advices.filter((a: any) => a.level === 'WARNING' || a.level === 'CRITICAL')
  for (const w of warnAdvices)
    warnings.push({ level: 'warn', message: w.message ?? String(w), hint: w.action })
  log.step(
    1,
    'Analyze',
    `${pivots} pivot${pivots !== 1 ? 's' : ''}${warnAdvices.length ? ` · ${warnAdvices.length} warnings` : ''}`,
    d2
  )
  ...
  const { durationMs: d5 } = timed(() => {
    const useCases: UseCase[] = fs.existsSync(files.useCases)
  ...
  log.step(
    5,
    'Compile',
    `${compileStats.totalRoutes} routes (${compileStats.physical ?? '?'} physical · ${compileStats.semantic ?? 0} semantic · ${compileStats.composed ?? 0} composed)`,
    d6
  )
      ...
      try {
        ...
        for (const [key, val] of Object.entries(devRoutes)) {
          if (dictData.routes[key]) {
            dictData.routes[key] = { ...dictData.routes[key], ...(val as object) }
          }
        }
      } catch { /* non bloquant */ }
    }
  ...
  log.success(
    path.relative(cwd, files.compiled).replace(/\\/g, '/'),
