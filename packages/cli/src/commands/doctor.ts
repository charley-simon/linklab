...
function printCheck(check: Check, indent = '  '): void {
...
async function runDoctor(
  ...
  let config: any = null
  let outDir: string = path.resolve(cwd, `linklab/${alias}`)
  ...
  if (!fs.existsSync(configPath)) {
    ...
    try {
      ...
      printCheck({ label: 'config illisible', status: 'error', detail: (e as Error).message })
      errors++
    }
  }
  ...
  if (errors === 0 && warnings === 0) {
    ...
    if (errors > 0) console.log(`  ${chalk.red('✖')}  ${errors} erreur${errors > 1 ? 's' : ''}`)
    if (warnings > 0)
...
export async function doctor(options: { alias?: string } = {}): Promise<void> {
