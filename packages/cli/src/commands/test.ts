...
export async function test(
  ...
  if (fs.existsSync(ucOverPath)) {
    ...
    for (const uc of override) byId.set(uc.id, uc)
  ...
  for (let i = 0; i < useCases.length; i++) {
    ...
    if (pct !== lastPct) {
      ...
      process.stdout.write(
        `\r  [${bar}] ${label.padStart(8)}  ok=${ok} empty=${empty} err=${errors}`
      )
      lastPct = pct
    }
  ...
  if (emptyRoutes.length > 0 && emptyRoutes.length <= 20) {
    ...
    if (emptyRoutes.length > 10) console.log(`    ... et ${emptyRoutes.length - 10} autres`)
  }
