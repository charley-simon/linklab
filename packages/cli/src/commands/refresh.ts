...
export async function refresh(options: { alias?: string } = {}): Promise<void> {
  ...
  try {
    ...
    console.error(`\n  ${chalk.red('✖')}  build échoué : ${(e as Error).message}\n`)
    process.exit(1)
  }
  ...
  try {
    ...
    console.log(chalk.red('✖'))
    console.error(`\n  ${chalk.red('✖')}  generate échoué : ${(e as Error).message}\n`)
    process.exit(1)
  }
  ...
  try {
    ...
    process.stdout.write('\x1B[1A\x1B[2K')
    console.log(
      `  ${chalk.dim('③')} test          ` + chalk.green('✔') + chalk.dim(`  ${elapsed3}`)
    )
  }
  ...
  try {
    ...
    console.log(chalk.red('✖'))
    console.error(`\n  ${chalk.red('✖')}  train échoué : ${(e as Error).message}\n`)
    process.exit(1)
  }
  ...
  try {
    ...
    console.log()
    console.log(
      `  ${chalk.green('✔')}  ${routes.length} routes` +
