...
export async function generate(options = {}) {
    ...
    const useCases = [];
    let idCounter = 0;
    const makeId = (from, to, suffix = '') => `UC-${alias}-${from}-${to}${suffix ? '-' + suffix : ''}-${++idCounter}`;
    // ── 1. Routes physiques ───────────────────────────────────────────────────
    const physical = routes.filter((r) => !r.semantic);
    ...
    const preComposed = routes.filter((r) => r.composed);
    ...
    console.log(`     ${physical_count} physiques · ${semantic_count} sémantiques · ${composed_count} composés`);
    console.log();
