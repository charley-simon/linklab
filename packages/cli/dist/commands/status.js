...
function statusLine(label, value, extra, ok = true) {
    const C = ok ? chalk.green : chalk.yellow;
    console.log(`  ${chalk.dim(label.padEnd(36))} ${C(value.padEnd(10))} ${extra ? chalk.dim(extra) : ''}`);
}
// ── Commande ──────────────────────────────────────────────────────────────────
export async function status(options = {}) {
    ...
    if (compiled.exists) {
        try {
            ...
            console.log(`  ${chalk.green('✔')}  ${routes.length} routes compiled` +
    ...
    if (fs.existsSync(testGenPath)) {
        ...
        console.log(`  ${chalk.green('✔')}  Test : ${report.ok}/${report.total} OK (${pct}%) — ${report.durationMs}ms`);
    }
    else {
