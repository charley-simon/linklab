 ...
 */
function resolveBreadcrumb(rpath, semLabel, compiled, dictionary) {
    ...
    if (semLabel) {
        ...
        const match = routes.find((r) => r.label === semLabel);
        if (match) {
            const from = match.from, to = match.to;
            const dictKey = `${from}→${to}[${semLabel}]`;
            const dictEntry = dictRoutes[dictKey];
            if (dictEntry?.label)
                return dictEntry.label;
        }
    ...
    const matchByPath = routes.find((r) => r.primary?.path && r.primary.path.join('→') === pathKey);
    if (matchByPath?.label && typeof matchByPath.label === 'string') {
        const from = matchByPath.from, to = matchByPath.to, label = matchByPath.label;
...
export async function repl(options = {}) {
    ...
    const entities = graph.entities.map((e) => e.id);
    ...
    const rootLabels = (() => {
        const labels = new Set([...entities]);
        const routes = (compiled.routes ?? []);
        for (const r of routes) {
            if (!r.semantic || !r.label)
                continue;
            const label = r.label;
    ...
    function completer(line) {
        ...
        if (line.startsWith('links ')) {
            ...
            const hits = entities.filter((e) => e.startsWith(partial));
            return [hits.map((e) => `links ${e}`), line];
        }
        ...
        const chainMatch = line.match(new RegExp(`^${alias}\\.([a-zA-Z_][a-zA-Z0-9_]*)\\([^)]*\\)\\.(.*)$`));
        if (chainMatch) {
            ...
            const fromLinks = links.map((l) => l.label);
            // Ajouter les filtres sémantiques (singular) : director, actor...
            const semFilters = (compiled.routes ?? [])
                .filter((r) => r.semantic && r.from === entity && !r.composed)
                .map((r) => {
                const lbl = r.label;
                return lbl.endsWith('_in') ? lbl.slice(0, -3) : lbl;
            });
            const allLabels = [...new Set([...fromLinks, ...semFilters])];
            const hits = allLabels.filter((l) => l.startsWith(partial));
            const prefix = `${alias}.${entity}(${chainMatch[0].match(/\(([^)]*)\)/)?.[1] ?? ''}).`;
            return [hits.map((l) => `${prefix}${l}`), line];
        }
        const domainMatch = line.match(new RegExp(`^${alias}\\.(.*)$`));
        if (domainMatch) {
            const partial = domainMatch[1];
            const hits = rootLabels.filter((s) => s.startsWith(partial));
            return [hits.map((s) => `${alias}.${s}`), line];
        }
...
`);
        ...
        if (linksMatch) {
            ...
            links
                .filter((l) => !l.semantic)
                .forEach((l) => console.log(`  ${c(C.cyan, l.label)}`));
            links
                .filter((l) => l.semantic)
                .forEach((l) => console.log(`  ${c(C.magenta, `${l.label} → ${l.to}`)}`));
            console.log();
            rl.prompt();
            return;
        }
        ...
        if (routesMatch) {
            try {
                ...
                console.log(c(C.red, `  Erreur : ${e.message}`));
            }
            rl.prompt();
            return;
        }
