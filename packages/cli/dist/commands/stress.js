...
function memTrend(passes) {
    ...
    if (avgLast - avgFirst > 5)
        return 'increasing';
    if (avgFirst - avgLast > 5)
        return 'decreasing';
    return 'stable';
}
// ── Exécution d'un use case ───────────────────────────────────────────────────
async function runUseCase(uc, engine, compiled, dataset, provider) {
...
function renderDashboard(passIdx, totalRuns, passes, routeMap, useCases, mode, vu, thinkMs, slowMs, critMs, watch, dashLines) {
    if (dashLines > 0)
        clearLines(dashLines);
    const lines = [];
    ...
    lines.push(`  ${chalk.dim('│')} Heap    : ${chalk.cyan(heapStr + ' MB')}  ${trendIcon}${leakWarn}`.padEnd(70) + chalk.dim('│'));
    lines.push(`  ${chalk.dim('│')} Routes  : ${chalk.green(`✔ ${ok}`)}  ${chalk.yellow(`⚠ ${slow}`)}  ${chalk.red(`✖ ${crit}`)}  Err: ${lastPass?.errors ?? 0}`.padEnd(70) + chalk.dim('│'));
    lines.push(`  ${chalk.dim('│')} Avg     : ${chalk.white(avgAll + 'ms')}   P95: ${chalk.yellow(p95All + 'ms')}   P99: ${chalk.red(p99All + 'ms')}`.padEnd(70) + chalk.dim('│'));
    lines.push(`  ${chalk.dim('└' + '─'.repeat(57) + '┘')}`);
    lines.push('');
    // Top 5 routes lentes
    ...
    if (routeAvgs.length > 0) {
        ...
        for (const r of routeAvgs) {
            const icon = r.avg >= critMs ? chalk.red('✖') : r.avg >= slowMs ? chalk.yellow('⚠') : chalk.green('✔');
            const desc = r.uc?.description ?? r.id;
            lines.push(`    ${icon}  ${chalk.dim(desc.slice(0, 45).padEnd(45))}  ${chalk.white(r.avg + 'ms')}`);
        }
    }
    lines.push('');
...
export async function stress(options = {}) {
    ...
    if (fs.existsSync(testGenPath)) {
        ...
        const okIds = new Set(report.results.filter((r) => r.status === 'ok').map((r) => r.id));
    ...
    console.log();
    console.log(`  ${chalk.dim(`${useCases.length} routes · mode: ${MODE}${options.watch ? ' · watch' : ''}${VU > 1 ? ` · ${VU} VU · think: ${THINK_MS}ms` : ''}`)}\n`);
    // State
    const routeMap = new Map(); // id → [durations]
    const passes = [];
    ...
    while (running && passIdx < TOTAL_RUNS) {
        ...
        if (MODE === 'concurrent') {
            ...
            const vuPromises = Array.from({ length: VU }, async (_, vuIdx) => {
                ...
                for (const uc of useCases) {
                    ...
                    process.stdout.write(`\r  ${chalk.dim(`[${bar}]`)} ${ucDone}/${useCases.length * VU}  heap: ${heapMB()}MB  `);
                    if (THINK_MS > 0) {
            ...
            for (const uc of useCases) {
                ...
                const bar = '█'.repeat(pct) + '░'.repeat(20 - pct);
                const passStr = TOTAL_RUNS === Infinity ? `passe ${passIdx}` : `passe ${passIdx}/${TOTAL_RUNS}`;
                process.stdout.write(`\r  ${chalk.dim(`[${bar}]`)} ${ucDone}/${useCases.length}  ${chalk.dim(passStr)}  heap: ${heapMB()}MB  `);
            }
        }
        ...
        if (options.watch || options.load || options.concurrent) {
            dashLines = renderDashboard(passIdx, TOTAL_RUNS, passes, routeMap, useCases, MODE, VU, THINK_MS, SLOW_MS, CRIT_MS, options.watch ?? false, dashLines);
        }
    }
    if (options.watch) {
        process.stdin.pause();
    ...
    // ── Résumé terminal ────────────────────────────────────────────────────────
    console.log(`  ${chalk.bold.white('Résultats')}  ·  ${passIdx} passes  ·  ${Math.round((Date.now() - globalStart) / 1000)}s`);
    console.log();
    console.log(`  ${chalk.green('✔')}  Rapides    (< ${SLOW_MS}ms)   : ${chalk.green(String(ok).padStart(5))}` +
        chalk.dim(` (${Math.round((ok / routeMetrics.length) * 100)}%)`));
    console.log(`  ${chalk.yellow('⚠')}  Lentes     (${SLOW_MS}-${CRIT_MS}ms) : ${chalk.yellow(String(slow).padStart(5))}` +
        chalk.dim(` (${Math.round((slow / routeMetrics.length) * 100)}%)`));
    console.log(`  ${chalk.red('✖')}  Critiques  (> ${CRIT_MS}ms)  : ${chalk.red(String(critical).padStart(5))}` +
        chalk.dim(` (${Math.round((critical / routeMetrics.length) * 100)}%)`));
    console.log();
    console.log(`  Mémoire : ${chalk.cyan(startHeap + ' MB')} → ${chalk.cyan(endHeap + ' MB')}` +
    ...
    if (routeMetrics.length > 0 && (slow > 0 || critical > 0)) {
        ...
        for (const r of routeMetrics.slice(0, 5)) {
            ...
            console.log(`    ${icon}  ${chalk.dim(r.description.slice(0, 48).padEnd(48))}` +
