...
export async function explore(options = {}) {
    ...
    const args = ['--compiled', compiledPath];
        ...
        if (config?.roots?.length) {
            ...
            try {
                ...
                const allRoots = c.nodes
                    .map((n) => n.id)
                    .filter((id) => !pivotPattern.test(id));
                if (allRoots.length)
                    roots = allRoots.join(',');
            }
            catch { }
    ...
    if (!options.label) {
        try {
            const c = JSON.parse(fs.readFileSync(compiledPath, 'utf-8'));
            if (c.scenario)
                args.push('--label', c.scenario);
        }
        catch { }
    }
    // ── Lancer le TUI ─────────────────────────────────────────────────────────
    console.log();
    console.log(`  ${chalk.bold.white('linklab explore')}  ·  ${chalk.gray(path.relative(cwd, compiledPath))}`);
    console.log();
