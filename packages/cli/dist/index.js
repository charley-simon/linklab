...
async function main() {
    ...
    switch (command) {
        ...
        case 'stress':
            await stress({
                alias: alias ?? flags['alias'],
                runs: flags['runs'] ? parseInt(flags['runs']) : undefined,
                load: flags['load'],
                concurrent: flags['concurrent'],
                vu: flags['vu'] ? parseInt(flags['vu']) : undefined,
                think: flags['think'] ? parseInt(flags['think']) : undefined,
                watch: flags['watch'],
                slowMs: flags['slow-ms'] ? parseInt(flags['slow-ms']) : undefined,
                criticalMs: flags['critical-ms'] ? parseInt(flags['critical-ms']) : undefined
            });
            break;
        case 'repl':
            await repl({
        ...
        case 'server':
            ...
            break;
        case 'doctor':
            await doctor({ alias: alias ?? flags['alias'] });
            break;
        case 'status':
            await status({ alias: alias ?? flags['alias'] });
            break;
        case 'diff':
            await diff({ alias: alias ?? flags['alias'] });
            break;
        case 'docs':
            await docs({ alias: alias ?? flags['alias'] });
            break;
        case 'help':
        case '--help':
        case '-h':
